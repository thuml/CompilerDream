...
class TensorBoardOutputPytorch:
    ...
    def __call__(self, summaries):
        ...
        for (step, name), value in scalar_summaries.items():
            if name == 'eval_reduction' or name == 'eval_return' or name == 'val_reduction' or name == 'val_return':
                self._writer.add_scalar("scalars/" + name + "_mean", np.mean(value), step)
                self._writer.add_scalar("scalars/" + name + "_geomean", geom_mean(value), step)
                self._stats[name + "_mean"] = np.mean(value)
                self._stats[name + "_geomean"] = geom_mean(value)
            else:
                self._writer.add_scalar("scalars/" + name, np.mean(value), step)
                self._stats[name] = np.mean(value)
